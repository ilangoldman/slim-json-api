...
class EmprestimoDBO extends DBO {
    ...
    public function __construct($db) {
        ...
        $this->setFK(['empresa']);
        $this->setRelations(["investimento","parcela","detalhe"]);
    ...
    public function allowAccess($userId,$userType,$itemId,$method) {
        if ($userType != "empresa" && $method != "get")
            return false;

        $this->read($itemId);
        $dbo = $this->controller->empresa();

        return $dbo->allowAccess($userId,$userType,$this->empresa,$method);        
    }
